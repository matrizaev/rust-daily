...
- Return `Result<T, LayerError>` from fallible application, runner, workspace, configuration, and API helpers. Do not collapse internal errors into strings until crossing an external boundary such as HTTP JSON or logs. API errors should preserve typed variants internally and serialize structured payloads such as `{ code, error, details }` at the HTTP boundary.
